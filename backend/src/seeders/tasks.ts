...
export async function seedTasks() {
    const tasks = [
        ...
        },
        {
            ...
            description: "Complete the team's progress slides and email them the head of department", 
            priority: 5, 
            completed: true
        },
        {
    ...
    for (const task of tasks) {
        await Task.findOrCreate({
            where: { name: task.name, description: task.description, priority: task.priority, completed: task.completed }
        })
    }
